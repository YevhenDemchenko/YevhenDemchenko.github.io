...
let config = {
    SIM_RESOLUTION: 256,                //резкость закручивания возле указателя
    DYE_RESOLUTION: 1024,                //качество пикселей
    DENSITY_DISSIPATION: 0.97,          //прозрачность
    VELOCITY_DISSIPATION: 0.98,         //размер возле мыши и скорость исчезновения
    PRESSURE_DISSIPATION: 0.8,          //шум
    PRESSURE_ITERATIONS: 20,
    CURL: 30,                           //разность цвета
    SPLAT_RADIUS: 0.9,                  //размер по бокам
    SHADING: true,
...
var displayShadingShader = compileShader(gl.FRAGMENT_SHADER, `
    ...
    void main () {
        ...
        float diffuse = clamp(dot(n, l) + 3.7, 3.7, 1.0);         //густота жидкости. чем выше значение больше похоже на дым 0.7 жидкость 3.7 дым
        C.rgb *= diffuse;
...
var splatShader = compileShader(gl.FRAGMENT_SHADER, `
    ...
    void main () {
        vec2 p = vUv - point.xy;
        p.x *= aspectRatio;                                             //меняется плотность фигуры +3,0
        vec3 splat = exp(-dot(p, p) / radius + 0.1) * color;            //добавить + 1.0 к радиусу будет ярче и больше
...
var curlShader = compileShader(gl.FRAGMENT_SHADER, `
    ...
    void main () {
        ...
        float vorticity = R - L - T + B;                     //прикольные еффекты при изменении знаков
...
var vorticityShader = compileShader(gl.FRAGMENT_SHADER, `
    ...
    void main () {
        ...
        vec2 force = 0.5 * vec2(abs(T) - abs(B), abs(R) - abs(L));  //изменить знаки смешивание с одной стороны
        force /= length(force) + 0.0001;
        force *= curl * C;
        force.y *= -2.5;                                            //меняет густость картинки -1.5
...
function step (dt) {
    ...
    gl.uniform2f(vorticityProgram.uniforms.texelSize, 5.0 / simWidth, 5.0 / simHeight);  //3микс цвета при увеличении чисел более гладкий
    ...
    gl.uniform2f(divergenceProgram.uniforms.texelSize, 1.0 / simWidth, 1.0 / simHeight);  //всё в пикселях при увеличении
    ...
    gl.uniform2f(pressureProgram.uniforms.texelSize, 1.0 / simWidth, 1.0 / simHeight);  //всё в шуму при увеличении
    ...
    gl.uniform2f(advectionProgram.uniforms.texelSize, 0.5 / simWidth, 0.5 / simHeight);         //дальность отрисовки
    if (!ext.supportLinearFiltering)
...
function splat (x, y, dx, dy, color) {
    ...
    gl.uniform1f(splatProgram.uniforms.aspectRatio, canvas.width / canvas.height);      //если умножить канву на ноль то будет горизонтальная полоса
    gl.uniform2f(splatProgram.uniforms.point, x / canvas.width, 1.0 - y / canvas.height);
    gl.uniform3f(splatProgram.uniforms.color, dx, -dy, 1.0);
    gl.uniform1f(splatProgram.uniforms.radius, config.SPLAT_RADIUS / 100.0);            //размер            
    blit(velocity.write.fbo);
    velocity.swap();
...
function multipleSplats (amount) {
    for (let i = 0; i < amount; i++) {
        ...
        color.g *= 10.0;
        color.b *= 10.0;
        const x = canvas.width * Math.random();   //сколько штук появляется при открытии страницы
        const y = canvas.height * Math.random();
        const dx = 1000 * (Math.random() - 0.5);
        const dy = 1000 * (Math.random() - 0.5);
        //splat(x, y, dx, dy, color);                //брызги вначале
    }
}
//////////////////////////////////////////////////////////////////////////////движение мышки
...
canvas.addEventListener('mousemove', e => {
    ...
    pointers[0].dx = (e.offsetX - pointers[0].x) * 5.0;     //эффект розмытия уменьшается при 1.0
